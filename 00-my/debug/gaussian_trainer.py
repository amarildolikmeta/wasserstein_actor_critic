...
class GaussianTrainer(SACTrainer):
    ...
    def train_from_torch(self, batch):
        ...
        if self.share_layers:
            ...
            raise ValueError('No shared layers anymore')
        else:
            # q network loss
            q_target = self.reward_scale * rewards + \
                       (1. - terminals) * self.discount * target_q_values
            ...
            # ADD
            f_obs = torch.FloatTensor(obs.shape[0], obs.shape[1]).uniform_(-1, 1)
            f_actions = torch.FloatTensor(actions.shape[0], actions.shape[1]).uniform_(-1, 1)
            f_std_preds = self.std(f_obs, f_actions)
            f_std_target = self.std(f_obs, f_actions)
            # ADD END
            ...
            std_target = torch.clamp(std_target, 0, self.std_init)
            std_loss = self.qf_criterion(std_preds, std_target.detach())
            std_loss += self.qf_criterion(f_std_preds, f_std_target.detach()) # ADDED
    ...
    def random_std_increase(self, batch_size, obs_dim, ac_dim):
        ...
        if self.std_inc_init:
            ...
            std_target = std_target.detach()
        std_loss = self.qf_criterion(std_preds, std_target)
        self.q_optimizer.zero_grad()
        std_loss.backward(retain_graph=True)
        self.q_optimizer.step()
