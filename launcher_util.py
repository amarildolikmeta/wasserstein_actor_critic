...
def run_experiment_here(
    ...
    np.random.seed(seed) 
    random.seed(seed)
    variant['seed'] = seed # add actual seed to variant

    log_dir = variant['log_dir'] + 's' + str(seed) + '/'
    ...
    setup_logger(
        ...
        log_dir=log_dir,
        variant=variant # FIXME: only debug
    )
    ...
    logger.log_variant(variant_log_path, variant) # save variant
...
def setup_logger(
    ...
    logger.set_keep_first(keep_first)
    logger.set_log_tabular_only(log_tabular_only)
    
    logger.set_alg(variant) # DEBUG: FIXME
