...
def plot_heatmaps(base_dir):
	...
	histog_arr = np.zeros(shape=[len(itrs), 41, 41], dtype=np.int32)
	...
	ori_min_state = -4 
	ori_max_state = 4 
	ori_min_action = -1 # CHANGE
	ori_max_action = 1 # CHANGE
	delta_state = 0.05
	delta_action = 0.05
	...
	for itr_i, itr in enumerate(itrs):
		...
		for i in range(len(xs)): # states
			...
			opt_policy[i] = eval_env.get_opt_action(xs[i])
		...
		if sampled:
			histog = np.histogram2d(a3d[itr,:,0],a3d[itr,:,1], bins=[41,41], range=[[-1, 1], [-1, 1]])[0]
			if not particle:
		...
		ytl = 10
		nyt = int((len(ya))/ytl)+1

		# save dir
		output_name = os.path.join(base_dir, 'plots')
		debug1 = 0
		...
		if vmax_std >= 0:
			JG10 = sns.heatmap(data=data_heatmap, xticklabels = xtl, yticklabels = ytl, vmin=0, vmax=vmax_std, cmap="jet_r")
		else:
			JG10 = sns.heatmap(data=data_heatmap, xticklabels = xtl, yticklabels = ytl, cmap="jet_r")

		JG10.set_xticklabels(np.linspace(ori_min_state,ori_max_state,nxt))
		JG10.set_yticklabels(np.linspace(ori_min_action,ori_max_action,nyt))

		JG10b = sns.lineplot(x=np.linspace(0,len(xs),len(xs)),y=max_std, color='white')

		ax.set_xlabel(r'$state$', fontsize=AXIS_FONT_SIZE)
		#ax.set_ylabel(f'{hp_names[hp0]}', fontsize=AXIS_FONT_SIZE)
		ax.set_ylabel(r'$action$', fontsize=AXIS_FONT_SIZE)
		...
		ax.set_title(r'$\sigma$', fontdict={'fontsize':TITLE_SIZE})
		...
		plt.tight_layout()
		fig.text(0.5, -0.04, 'Our regularization', fontdict={'fontsize':TITLE_SIZE}, wrap=True, horizontalalignment='center', fontsize=TICKS_FONT_SIZE)
		fig.savefig(os.path.join(output_name, 'std_poster' + str(itr) + '.png'), bbox_inches = "tight")
		fig.savefig(os.path.join(output_name, 'std_poster' + str(itr) + '.pdf'), bbox_inches = "tight", format='pdf')
		...
		JG00 = sns.heatmap(data=mean_heatmap, xticklabels = xtl, yticklabels = ytl, cmap="jet_r")

		JG00.set_title('Mean')
		JG00.set_xticklabels(np.linspace(ori_min_state,ori_max_state,nxt))
		JG00.set_yticklabels(np.linspace(ori_min_action,ori_max_action,nyt))

		JG00b = sns.lineplot(x=np.linspace(0,len(xs),len(xs)),y=opt_policy_ce, color='white')
		JG00c = sns.lineplot(x=np.linspace(0,len(xs),len(xs)),y=((tar_policy+1)*(len(ya)-1)/2), color='black')
		JG00d = sns.lineplot(x=np.linspace(0,len(xs),len(xs)),y=((opt_policy+1)*(len(ya)-1)/2), color='gray')
		...
		ax.xaxis.offsetText.set_fontsize(TICKS_FONT_SIZE+2)
		ax.yaxis.offsetText.set_fontsize(TICKS_FONT_SIZE+2)
		ax.set_title('Mean', fontdict={'fontsize':TITLE_SIZE})
		cbar = ax.collections[0].colorbar
		# here set the labelsize by 20
		cbar.ax.tick_params(labelsize=TICKS_FONT_SIZE)
		ax.tick_params(labelsize=TICKS_FONT_SIZE)
		
		fig.savefig(os.path.join(output_name, 'mean' + str(itr) + '.png'))
		fig.savefig(os.path.join(output_name, 'mean' + str(itr) + '.pdf'), format='pdf')
		plt.close('all') 
		...
		JG20 = sns.heatmap(data=upbd_heatmap, xticklabels = xtl, yticklabels = ytl, cmap="jet_r")

		JG20.set_title('Upper Bound')
		JG20.set_xticklabels(np.linspace(ori_min_state,ori_max_state,nxt))
		JG20.set_yticklabels(np.linspace(ori_min_action,ori_max_action,nyt))

		JG20b = sns.lineplot(x=np.linspace(0,len(xs),len(xs)),y=opt_policy_ub, color='white')
		JG20c = sns.lineplot(x=np.linspace(0,len(xs),len(xs)),y=((policy+1)*(len(ya)-1)/2), color='black')
		...
		ax.xaxis.offsetText.set_fontsize(TICKS_FONT_SIZE+2)
		ax.yaxis.offsetText.set_fontsize(TICKS_FONT_SIZE+2)
		ax.set_title('UpperBound', fontdict={'fontsize':TITLE_SIZE})
		cbar = ax.collections[0].colorbar
		# here set the labelsize by 20
		cbar.ax.tick_params(labelsize=TICKS_FONT_SIZE)
		ax.tick_params(labelsize=TICKS_FONT_SIZE)
		
		fig.savefig(os.path.join(output_name, 'ub' + str(itr) + '.png'))
		fig.savefig(os.path.join(output_name, 'ub' + str(itr) + '.pdf'), format='pdf')
		plt.close('all') 

		# cum histo
		fig, ax = plt.subplots(1, 1, figsize=(20, 12))

		JG11 = sns.heatmap(data=cum_histo, xticklabels = xtl, yticklabels = ytl, cmap="jet_r")
		# JG11 = sns.heatmap(data=histog_ori, xticklabels = xtl, yticklabels = 8, cmap="jet_r", ax=ax[1,1])

		ax.set_title('Cumulative sampled tuples', fontdict={'fontsize':TITLE_SIZE})
		#JG11.set_title('sampled')
		JG11.set_xticklabels(np.linspace(ori_min_state,ori_max_state,nxt))
		JG11.set_yticklabels(np.linspace(ori_min_action,ori_max_action,nyt))
		...
		fig.savefig(os.path.join(output_name, 'cum_histo' + str(itr) + '.png'))
		fig.savefig(os.path.join(output_name, 'cum_histo' + str(itr) + '.pdf'), format='pdf')
		plt.close('all') 
		...
		JG01 = sns.heatmap(data=histog, xticklabels = xtl, yticklabels = ytl, cmap="jet_r")
		#JG01.set_title('sampled [log_2]')
		ax.set_title('Sampled tuples in current epoch', fontdict={'fontsize':TITLE_SIZE})
		JG01.set_xticklabels(np.linspace(ori_min_state,ori_max_state,nxt))
		JG01.set_yticklabels(np.linspace(ori_min_action,ori_max_action,nyt))
		...
		fig.savefig(os.path.join(output_name, 'sampled' + str(itr) + '.png'))
		fig.savefig(os.path.join(output_name, 'sampled' + str(itr) + '.pdf'), format='pdf')
		plt.close('all') 	
