...
save = True
sampled = True
base_dir = './data/riverswim/25/sac_/rs06'
...
def plot_heatmaps(base_dir):
	...
	#delta_state = 0.5
	ori_max_state = 25
	delta_state = (max_state - min_state) / ori_max_state / 2
	#delta_state = 2/12/4 
	delta_action = 0.05
	...
	for itr_i, itr in enumerate(itrs):
		...
		for i in range(len(xs)): # states
			...
			ac_2, *_ = trainer.policy_2.forward(torch_ify(np.reshape(xs[i], (1,1))), deterministic=True)
			policy_2[i] = np_ify(ac_2[0,0])
		...
		if sampled and not Debug: 
			...
			JG01.set_xticklabels(np.round(np.linspace(0,20,5)))
			...
			if particle:
				...
				JG11.set_xticklabels(np.round(np.linspace(0,20,5)))
		...
		JG20d = sns.lineplot(x=np.linspace(0,len(xs),len(xs)),y=((policy_2+1)*(len(ya)-1)/2), color='gray', ax=ax[2,0])
		
		# JG21b = sns.lineplot(x=np.linspace(0,61,61),y=policy_star*(opt_qf.shape[0]-1), color='white', ax=ax[2,1]) # FM: parametrize
